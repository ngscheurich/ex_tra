;;; ex_tra.el --- Region transform via elixir scripts -*- lexical-binding: t; -*-

(defvar elixir-transform-bin
  (let ((elisp-dir (file-name-directory (or load-file-name buffer-file-name))))
    (expand-file-name "ex_tra" elisp-dir))
  "Path to the `ex_tra` binary for executing Elixir transforms.")
...
(defun ex_tra ()
  ...
  (let* ((cmd (format "%s list_transforms" elixir-transform-bin))
    ...
    (let* ((choice (completing-read "Elixir Transform: " commands))
           ...
           (cmd (format "%s %s \"%s\"" elixir-transform-bin function-name region-elixir-str))
           (start (region-beginning))
           (end (region-end))
           (orig-buf (current-buffer))
           (raw-output (progn (message "Evaluating: %s" cmd) (shell-command-to-string cmd)))
