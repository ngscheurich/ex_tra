defmodule AtomifyMap do
  @moduledoc """
  Converts string keys to atom keys in maps found within Elixir AST or runtime maps.
  """

  @doc """
  Takes an Elixir AST and recursively converts all string keys to atom keys
  in any maps found within the AST.

  ## Examples

      iex> ast = quote do: %{"foo" => 1, "bar" => %{"baz" => 2}}
      iex> AtomifyMap.atomify_map(ast)
      {:%{}, [], [{:foo, 1}, {:bar, {:%{}, [], [{:baz, 2}]}}]}
      ...
      iex> ast = quote do: [1, %{"key" => "value"}, 3]
      iex> AtomifyMap.atomify_map(ast)
      [1, {:%{}, [], [{:key, "value"}]}, 3]
  """
  def atomify_map(term) when is_binary(term) do
    try do
  ...
  defp convert_map_pair({key, value}) when is_binary(key) do
  ...
  ## Examples
      ...
      iex> AtomifyMap.convert_any(%{"foo" => 1, "bar" => %{"baz" => 2}})
      %{foo: 1, bar: %{baz: 2}}

      iex> ast = quote do: %{"foo" => 1, "bar" => %{"baz" => 2}}
      iex> AtomifyMap.convert_any(ast)
      {:%{}, [], [{:foo, 1}, {:bar, {:%{}, [], [{:baz, 2}]}}]}
  """
  def convert_any(term) when is_map(term) do
    # Handle runtime maps
    Map.new(term, fn
      {key, value} when is_binary(key) -> {String.to_atom(key), convert_any(value)}
